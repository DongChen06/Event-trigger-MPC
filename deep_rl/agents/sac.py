...
class Agent(object):
      ...
      while not (done or steps == max_step):
         ...
         if self.eval_mode:
            ...
            if self.steps > self.expl_before:
               ...
               action = np.random.sample()

            # Collect experience (s, a, r, s') using some policy
            next_obs, reward, done, _ = self.env.step(action)

            # Add experience to replay buffer
            # self.replay_buffer.add(obs, action, reward / 1000, next_obs, done)
   ...
   def test(self, max_step):
      ...
      # Keep interacting until agent reaches a terminal state.
      while not (done or steps == max_step):
         ob_history.append(obs)

         action, _, _ = self.policy(torch.Tensor(obs).to(self.device))
         action = action.detach().cpu().numpy()
         next_obs, reward, done, con = self.env.step(action)
         actions.append(action)
         total_reward += reward
         steps += 1
         obs = next_obs

      return steps, total_reward, np.array(ob_history), (np.array(actions), np.array(con))
